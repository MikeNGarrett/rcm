...
?>
	...
	<input type="hidden" id="gmw-submitted-<?php echo $gmw['form_id']; ?>" class="gmw-submitted" name="gmw-submitted" value="" />
	<input type="submit" id="gmw-submit-<?php echo $gmw['form_id']; ?>" class="gmw-submit" form_id="<?php echo $gmw['form_id'];?>" name="submit" value="<?php _e($subValue,'GMW'); ?>" />
	</div>
<?php
}
...
function gmw_shortcode($params) {
	...
	if ( isset($gmw['results_page']) && !empty($gmw['results_page']) ) $gmw['results_page'] = get_permalink($gmw['results_page']);
	...
	if ( isset( $_GET['action'] ) && $_GET['action'] == "wppl_post" && $gmw['form_id'] == $_GET['wppl_form'] && !isset( $params['widget']) ) :
		// when form submitted
